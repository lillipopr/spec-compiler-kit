...
| 维度 | 权重 | 评分要点 |
|------|------|---------|
| 应用层职责 | 18 分 | 用例编排、事务管理、调用领域层、发布事件、返回结果 |
| 用户行为 vs 系统行为 | 13 分 | 行为分类、决策树、特征区分 |
| 应用服务设计 | 22 分 | 服务命名、方法命名、服务结构 |
| 业务逻辑分层 | 13 分 | 应用层不包含业务逻辑、参数校验分层 |
| 事务管理 | 9 分 | 事务边界、事务粒度 |
| Command 和 DTO | 9 分 | Command 设计、DTO 设计 |
| 错误处理 | 3 分 | 错误分类、异常转换 |
| 事件处理设计 | 13 分 | 事件监听器识别、监听器设计、幂等性、异常处理（v2.1 新增） |
...
**检查项**：
- [ ] 使用"聚合名 + Application"格式（4分）：MembershipApplication、CouponApplication
...
**检查项**：
- [ ] 使用动词+名词（4分）：createMembership()、cancelMembership()、upgradeMembership()
...
**检查项**：
- [ ] 应用层管理事务边界（4分）：使用 @Transactional 注解
...
**评分标准**：
- 优秀（90-100分）：所有领域事件都有对应的监听器
- 良好（80-89分）：90%以上领域事件有对应监听器
- 及格（60-79分）：70%以上领域事件有对应监听器
- 不及格（<60分）：低于70%领域事件有对应监听器

**检查项**：
- [ ] 事件发布方有监听器（2分）：第二章中的事件发布都有监听器
- [ ] 监听器列表完整（1分）：每个事件都有对应监听器
...
**评分标准**：
- 优秀（90-100分）：所有监听器职责清晰
- 良好（80-89分）：90%以上监听器职责清晰
- 及格（60-79分）：70%以上监听器职责清晰
- 不及格（<60分）：低于70%监听器职责清晰
...
**评分标准**：
- 优秀（90-100分）：所有监听器包含完整处理步骤
- 良好（80-89分）：90%以上监听器处理步骤完整
- 及格（60-79分）：70%以上监听器处理步骤完整
- 不及格（<60分）：低于70%监听器处理步骤完整
...
**评分标准**：
- 优秀（90-100分）：所有监听器都有幂等性保证
- 良好（80-89分）：90%以上监听器有幂等性保证
- 及格（60-79分）：70%以上监听器有幂等性保证
- 不及格（<60分）：低于70%监听器有幂等性保证

**检查项**：
- [ ] 幂等键定义清晰（1分）：使用 eventId 作为幂等键
- [ ] 存储位置明确（1分）：Redis / DB
- [ ] 去重逻辑正确（1分）：处理前检查是否已处理
...
- application.md 8.1-8.5：事件处理设计原则（v2.1 新增）
